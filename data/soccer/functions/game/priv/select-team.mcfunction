...
## 白チームを設定
execute if data storage soccer {now:{w:red}} run scoreboard players set @a[team=red,scores={JOIN=1..}] _W 1
execute if data storage soccer {now:{w:blue}} run scoreboard players set @a[team=blue,scores={JOIN=1..}] _W 1
execute if data storage soccer {now:{w:yellow}} run scoreboard players set @a[team=yellow,scores={JOIN=1..}] _W 1
execute if data storage soccer {now:{w:green}} run scoreboard players set @a[team=green,scores={JOIN=1..}] _W 1
scoreboard players set @a[scores={_W=1}] PLAY 1

## 黒チームを設定
execute if data storage soccer {now:{b:red}} run scoreboard players set @a[team=red,scores={JOIN=1..}] _B 1
execute if data storage soccer {now:{b:blue}} run scoreboard players set @a[team=blue,scores={JOIN=1..}] _B 1
execute if data storage soccer {now:{b:yellow}} run scoreboard players set @a[team=yellow,scores={JOIN=1..}] _B 1
execute if data storage soccer {now:{b:green}} run scoreboard players set @a[team=green,scores={JOIN=1..}] _B 1
