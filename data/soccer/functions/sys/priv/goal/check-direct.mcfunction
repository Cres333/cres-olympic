...
## 白のゴール側に直接持ち込んだ
execute if entity @p[scores={_HAS=1},predicate=soccer:white-goal] run scoreboard players set $isB _ 1
...
## 黒のゴール側に直接持ち込んだ
execute if score $isGoal _ matches 0 if entity @p[scores={_HAS=1},predicate=soccer:black-goal] run scoreboard players set $isW _ 1
