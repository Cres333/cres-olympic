...
## カウントが残っているならループする
execute if score $count _ matches 1.. run schedule function battle:sys/priv/count 1s

## カウントが残っていないなら開始する
execute unless score $count _ matches 1.. run function battle:sys/priv/start
