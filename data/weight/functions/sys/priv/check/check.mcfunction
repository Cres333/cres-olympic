...
execute as @a[predicate=util:play] if score @s _CLICK > $value _CLICK run function weight:sys/priv/check/remove_life

## 金床を配置
function weight:sys/priv/check/set_anvil
...
## 金床を連打数に応じて持ち上げる
function weight:sys/priv/check/move_anvil
